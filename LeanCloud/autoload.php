<?php
/**
 * Enable autoload
 * 
 * Require this file to enable autoload SDK classes on the fly. If you're
 * using composer, it should be required automatically.
 *
 * @see https://github.com/php-fig/fig-standards/blob/master/accepted/PSR-4-autoloader-examples.md
 *
 * @param string $class The fully-qualified class name.
 * @return void
 */
spl_autoload_register(function ($class) {

    // project-specific namespace prefix
    $prefix = 'LeanCloud\\';

    // base directory for the namespace prefix
    $base_dir = __DIR__ . '/';
    //原来SDK是  $base_dir = __DIR__ . '/LeanCloud/';var_dump路径不对应该是去掉LeanCloud目录；否则会出错！特别加以注释；
    //同时应该在要引用的APP的index.php入口文件 加入loader;否则找不到LIB库； eg:require(__DIR__ . '/../../vendor/LeanCloud/autoload.php');

//var_dump($b);
    // does the class use the namespace prefix?
    $len = strlen($prefix);
    if (strncmp($prefix, $class, $len) !== 0) {
        // no, move to the next registered autoloader
        return;
    }

    // get the relative class name
    $relative_class = substr($class, $len);

    // replace the namespace prefix with the base directory, replace
    // namespace separators with directory separators in the relative
    // class name, append with .php
    $file = $base_dir . str_replace('\\', '/', $relative_class) . '.php';

    // if the file exists, require it
    if (file_exists($file)) {
        require $file;
    }
});

